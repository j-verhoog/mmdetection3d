...
class ModelComparator:
    ...
    def __init__(
        ...
        Args:
            ...
            pcd_files: List of point cloud files for inference
    ...
    def compare_activation_metric(
        ...
        try:
            ...
            print(f"  [Phase 1] Caching activations for Model A ({len(self.pcd_files)} samples)...")
            ...
            for i, pcd in enumerate(tqdm(self.pcd_files, desc="Model A")):
                acts = process_single_sample(model_a, pipeline_a, pcd, hooks_a)
            ...
            for i, pcd in enumerate(tqdm(self.pcd_files, desc="Model B")):
                acts_b = process_single_sample(model_b, pipeline_b, pcd, hooks_b)
    ...
    def compare_bn_metric(
        ...
        model_b_path: str,
        metric: MetricCalculator,
        pcd_path_for_init: Optional[str] = None
    ) -> Dict[str, float]:
        """
        ...
        Args:
            ...
            model_b_path: Path to checkpoint for model B
            metric: Metric calculator instance
            pcd_path_for_init: One PCD file to run through for BN initialization
        ...
        if pcd_path_for_init and len(self.pcd_files) > 0:
            pcd = self.pcd_files[0]
            with torch.no_grad():
                from comparison_utils import process_single_sample
                from comparison_utils import register_hooks, default_hook_filter
                
                hooks_a, handles_a = register_hooks(model_a, default_hook_filter)
                process_single_sample(model_a, pipeline_a, pcd, hooks_a)
                for h in handles_a:
                    h.remove()
                
                hooks_b, handles_b = register_hooks(model_b, default_hook_filter)
                process_single_sample(model_b, pipeline_b, pcd, hooks_b)
                for h in handles_b:
                    h.remove()
