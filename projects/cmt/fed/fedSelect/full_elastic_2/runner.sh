...
if [ "$START_ROUND" -eq 1 ]; then
    ...
    if [ -f "$START_MODEL_OVERRIDE/merged_${START_MODEL}.pth" ]; then
        echo "Found override checkpoint for Round $START_ROUND Model $START_MODEL. Using that instead of Round $((START_ROUND - 1)) merged model."
        MODEL_A="$START_MODEL_OVERRIDE/merged_A.pth"
        MODEL_B="$START_MODEL_OVERRIDE/merged_B.pth"
        MODEL_C="$START_MODEL_OVERRIDE/merged_C.pth"
        MODEL_D="$START_MODEL_OVERRIDE/merged_D.pth"
        MODEL_E="$START_MODEL_OVERRIDE/merged_E.pth"
    else
        echo "No override checkpoint found for Round $START_ROUND Model $START_MODEL. Defaulting to merged models from Round $((START_ROUND - 1))."
        MODEL_A="$WORK/round_$((START_ROUND - 1))/merged_A.pth"
        MODEL_B="$WORK/round_$((START_ROUND - 1))/merged_B.pth"
        MODEL_C="$WORK/round_$((START_ROUND - 1))/merged_C.pth"
        MODEL_D="$WORK/round_$((START_ROUND - 1))/merged_D.pth"
        MODEL_E="$WORK/round_$((START_ROUND - 1))/merged_E.pth"
    fi
fi
...
for ((i=START_ROUND; i<=NUM_ROUNDS; i++)); do
    ...
    # Fixed FedSelect ratio schedule (halved):
    case "$i" in
        11) RATIO="0.025" ;;
        12) RATIO="0.05" ;;
        13) RATIO="0.075" ;;
    ...
    echo "Using FedSelect Ratio: $RATIO for Round $i"

    run_merge "$CKPT_A" "$CKPT_B" "$CKPT_C" "$CKPT_D" "$CKPT_E" \
              "$MERGED_A" "$MERGED_B" "$MERGED_C" "$MERGED_D" "$MERGED_E" "$CURRENT_METHOD" "$RATIO"
