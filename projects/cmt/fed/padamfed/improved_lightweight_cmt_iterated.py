...
custom_imports = dict(
    imports=['projects.cmt.hooks.disable_gt_sampling', 'projects.cmt.hooks.force_stop', 'projects.cmt.hooks.PAdaMFed_Hook'], 
...
# does nothing but is required so we make it like this to avoid errors about missing optimizer config
optimizer = dict(
    type='SGD',
    lr=1.0,
    momentum=0.0,
...
optimizer_config = dict(
    type='PAdaMFedVRFp16OptimizerHook',
    loss_scale='dynamic',
    grad_clip=dict(max_norm=35, norm_type=2),
    custom_fp16=dict(
        pts_voxel_encoder=False,
        pts_middle_encoder=False,
        pts_bbox_head=False,
    ),
    client_id='ModelA',
    work_dir='/workspace/work_dirs/PAdaMFed_VR/client_states',
...
#     step_ratio_up=0.4)
# total_epochs = 2
evaluation = dict(interval=1, min_gt_count=5)
